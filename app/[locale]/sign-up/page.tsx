...
export default function SignUpPage() {
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        setLoading(true);
        try {
            const res = await (signUp.email as any)({ name, email, password });
            if (res.error) {
            ...
            <main className="flex-grow flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
                <div className="w-full max-w-md bg-white rounded-md shadow-sm border border-gray-100 p-8 relative z-10">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label htmlFor="name" className="block text-sm font-semibold text-gray-800 mb-1">{t("fullName")}</label>
                            <input id="name" name="name" type="text" maxLength={70}
                                placeholder={t("fullNamePlaceholder")} required disabled={loading}
                        ...
                        <div>
                            ...
                            <input id="phone" name="phone" type="tel" maxLength={15}
                                placeholder={locale === "fr" ? "Entrez votre numéro de téléphone" : "Enter your phone number"}
                                disabled={loading}
                                className="block w-full px-3 py-2 border border-gray-300 rounded-sm text-gray-900 focus:ring-1 focus:ring-[#1E3A8A] focus:border-[#1E3A8A] sm:text-sm transition-colors disabled:bg-gray-50 disabled:text-gray-500"
                            />
                        </div>
                        <div>
                            ...
                            <div className="relative">
                                <input id="password" name="password"
                                    ...
                                    placeholder={t("passwordPlaceholder")} required minLength={8} disabled={loading}
                        ...
                        <div>
                            <label htmlFor="confirmPassword" className="block text-sm font-semibold text-gray-800 mb-1">{t("confirmPassword")}</label>
                            <input id="confirmPassword" name="confirmPassword"
                                type={showPassword ? "text" : "password"}
                                placeholder={t("confirmPasswordPlaceholder")} required disabled={loading}
                                className="block w-full px-3 py-2 border border-gray-300 rounded-sm text-gray-900 focus:ring-1 focus:ring-[#1E3A8A] focus:border-[#1E3A8A] sm:text-sm transition-colors disabled:bg-gray-50 disabled:text-gray-500"
                            />
                        </div>
