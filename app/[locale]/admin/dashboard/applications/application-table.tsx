...
export default function ApplicationTable({
    ...
    const filteredApplications = applications.filter(app => {
        ...
        const matchesSearch =
            app.client.name.toLowerCase().includes(search) ||
            app.client.email.toLowerCase().includes(search) ||
     ...
     * Otherwise the user could remain on a page that no longer exists.
     */
    useEffect(() => {
        setPage(1);
    ...
    /*
     * Make sure the current page stays valid after deleting rows.
     */
    useEffect(() => {
        if (page > totalPages) {
            setPage(totalPages);
    ...
    const handleStatusChange = async (
        ...
        const res = await updateApplicationAction(appId, {
            status: newStatus
        });
                            ...
                            setApplications(prev => [
                                {
                                    ...
                                    type: newApp.applicationTemplateId
                                        ? "CUSTOM"
                                        : "GENERAL",
                                    agent: null
                                },
                        ...
                        {paginatedApplications.map(app => (
                            ...
                            >
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-[16px] lg:text-[18px] font-extrabold text-[#111827] leading-none mb-1.5">
                                                <TruncatedText
                                                    text={app.client.name}
                                                    maxLength={20}
                                                />
                                            </p>

                                            <p className="text-[14px] text-[#6B7280] font-mono font-bold">
                                                {t("idPrefix")}{" "}
                                                {app.id
                                                    .substring(0, 8)
                                                    .toUpperCase()}
                                ...
                                <td className="px-6 py-5">
                                    ...
                                    <p className="text-[12px] lg:text-[14px] font-bold uppercase tracking-tight text-[#6B7280] mt-1">
                                        {(app.type || "GENERAL").replace(
                                            "_",
                                            " "
                                        )}
                                    </p>
