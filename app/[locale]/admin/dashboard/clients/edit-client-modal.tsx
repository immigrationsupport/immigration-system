...
interface EditClientModalProps {
    ...
    currentName: string;
    currentEmail: string;
    onClientUpdated?: (updated: { id: string; name: string; email: string }) => void;
}
...
export default function EditClientModal({
    ...
    const t = useTranslations("adminClients.edit");
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogTrigger asChild>
                <button
                    className="p-2 text-[#374151] hover:text-[#1E3A8A] hover:bg-white rounded-lg transition-all"
                    title={t("tooltip")}
                >
                    <Edit2 size={20} />
                </button>
            ...
            <DialogContent className="sm:max-w-[450px] border-none shadow-2xl rounded-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-gray-500 font-medium">
                        {t("subtitlePrefix")}{" "}
                        <span
                            className="text-blue-600 font-bold inline-block max-w-[200px] truncate align-bottom"
                            title={currentName}
                        >
                            {currentName}
                        </span>
