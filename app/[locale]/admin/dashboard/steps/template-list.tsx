    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {templates.map((tpl) => (
                    <Card key={tpl.id} className="border-none shadow-lg rounded-2xl overflow-hidden group">
                        <CardContent className="p-5 flex items-start justify-between gap-3">
                            <Link href={`/admin/dashboard/steps/${tpl.id}`} className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 text-gray-400 mb-2">
                                    <ListOrdered className="h-4 w-4" />
                                    <span className="text-xs font-bold uppercase tracking-wide">{t("stepCount", { count: tpl.stepCount })}</span>
                                </div>
                                <p className="font-black text-gray-900 truncate">{tpl.name}</p>
            ...
            <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                <DialogContent className="sm:max-w-md rounded-2xl">
                    <DialogHeader>
                        <DialogTitle className="text-xl font-black text-[#1E3A8A]">{t("createDialogTitle")}</DialogTitle>
                    ...
                    <form onSubmit={handleCreate} className="space-y-4">
                        <div className="space-y-1.5">
                            ...
                            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder={t("namePlaceholder")} disabled={isPending} required />
                        </div>
                        <div className="space-y-1.5">
                        ...
                        <Button type="submit" disabled={isPending} className="w-full bg-[#1E3A8A] text-white hover:bg-blue-900 font-bold h-11 rounded-xl">
                            {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : t("createAndBuild")}
                        </Button>
                    </form>
                </DialogContent>
            </Dialog>
