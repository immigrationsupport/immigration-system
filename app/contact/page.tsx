...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitted(true); setForm({fullName:"",email:"",phone:"",message:""});
    } catch(err:any){ setError(err.message||"Failed to send. Try again."); } finally { setSubmitting(false); }
  };
            ...
            <div className="lg:col-span-3 bg-white rounded-2xl border border-[#eef0f8] p-10 shadow-sm">
              ...
              {submitted ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  {error && <div className="flex items-start gap-3 bg-red-50 border border-red-200 text-red-700 p-4 rounded-xl text-[13px]"><AlertCircle className="w-5 h-5 flex-shrink-0 mt-0.5"/><span>{error}</span></div>}
                  <div>
                    ...
                    <input type="text" id="fullName" value={form.fullName} onChange={handleChange} required maxLength={50} className={inp} placeholder="Enter your full name"/>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      ...
                      <input type="email" id="email" value={form.email} onChange={handleChange} required maxLength={50} className={inp} placeholder="Enter your email"/>
                    </div>
                    <div>
                      <label htmlFor="phone" className="block text-[12px] font-bold text-slate-600 uppercase tracking-[1px] mb-2">Phone Number</label>
                      <input type="tel" id="phone" value={form.phone} onChange={handleChange} className={inp} placeholder="Enter your phone number"/>
                    </div>
                  </div>
                  <div>
                    ...
                    <textarea id="message" value={form.message} onChange={handleChange} required rows={5} minLength={50} maxLength={1000} className={`${inp} resize-none`} placeholder="Describe your immigration needs..."/>
                  </div>
