...
export default function ApplicationStepper({ steps, applicationId, isPending, country }: ApplicationStepperProps) {
    ...
    const handleSubmitStep = async (stepId: string, type: string) => {
        ...
        if (type === "DOCUMENT_COLLECTION") {
            ...
            if (missing.length > 0) {
                setErrorModal(`Please upload the following required documents before submitting: ${missing.map(m => m.name).join(", ")}`);
                return;
            }
        }
        ...
        if (currentIdx > 0) {
            ...
            if (previousStep && previousStep.status !== "APPROVED") {
                setErrorModal(`You must complete Step ${currentIdx}: ${STEP_LABELS[previousStepType as keyof typeof STEP_LABELS]} before this one.`);
                return;
            }
        }
        ...
        <div className="space-y-8 pb-12">
            ...
            <div className="bg-white p-10 rounded-[50px] border border-gray-100 shadow-2xl shadow-gray-200/50">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
                    <div>
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] mb-2">Process Reliability</p>
                        <h3 className="text-4xl font-black text-[#1E3A8A] uppercase tracking-tighter">Application Status</h3>
                    </div>
                    <div className="text-left md:text-right">
                        ...
                        <p className="text-xs font-black text-emerald-500 uppercase tracking-[0.2em] mt-2 flex items-center gap-2 md:justify-end">
                            <CheckCircle2 size={14} /> {completedStepsCount} of {steps.length}  Milestones Validated
                        </p>
                    </div>
                </div>
                ...
                <tbody className="text-sm">
                    {APP_STEP_SEQUENCE.map((stepType, idx) => {
                        ...
                        const renderActionColumn = () => {
                            if (isCompleted) {
                                return <span className="text-green-600 font-bold tracking-tight">Completed</span>;
                            }
                            if (isLocked) {
                                return <span className="text-gray-400 font-medium">Locked</span>;
                            }
                            if (!dbStep) {
                                return <span className="text-gray-400 text-xs italic">Legacy Application</span>;
                            }

                            const getActionDescription = () => {
                                switch (stepType as string) {
                                    case "FEE_PAYMENT": return "Upload Payment Receipt";
                                    case "MEDICAL_EXAMINATION": return "Upload Medical Report";
                                    case "LANGUAGE_TEST_REGISTRATION": return "Upload Test Registration";
                                    case "LANGUAGE_TEST_RESULTS": return "Upload Test Results";
                                    case "CONTRACT_SIGNING": return "Upload Signed Contract";
                            ...
                            if (AGENT_ONLY_STEPS.includes(stepType as string)) {
                                if (stepType === "DIPLOMA_EQUIVALENCE") {
                                    return (
                                        ...
                                        >
                                            {expandedStepId === step.id ? "Close" : "Upload Proof"}
                                        </button>
                                    );
                                }
                                return <span className="text-[#1E3A8A] font-black italic text-[10px] uppercase tracking-widest bg-blue-50 px-3 py-1 rounded-full border border-blue-100">Handled by Agent Only</span>;
                            } else if (stepType === "LANGUAGE_TEST_REGISTRATION") {
                                if (hasCompletedLanguageTest === true) {
                                    return <span className="text-emerald-600 font-bold italic text-xs">Skipped (Test Completed)</span>;
                                }
                                return (
                                    <div className="space-y-3">
                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-tight">Have you completed a test?</p>
                                ...
                                if (hasCompletedLanguageTest === false) {
                                    return <span className="text-gray-400 italic text-[10px] uppercase font-black">Waiting for Registration</span>;
                                }
                            }
                            ...
                            if (stepType === "DOCUMENT_COLLECTION") {
                                return (
                                    <button 
                                        ...
                                        className="bg-blue-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 shadow-lg shadow-blue-100 transition-all active:scale-95"
                                    >
                                        {expandedStepId === step.id ? "Close Documents" : "Manage Documents"}
                                    </button>
                                );
                            }
                            ...
                            return (
                                <div className="flex flex-col gap-2">
                                    ...
                                    {uploadingStepId === step.id ? (
                                        <span className="text-blue-500 font-bold text-xs animate-pulse">Uploading...</span>
                                    ) : isUploadSuccess ? (
                                        ...
                                        <UploadButton
                                            ...
                                            onClientUploadComplete={(res) => handleUploadComplete(res, step.id, "OTHER", getActionDescription(), true)}
                                            ...
                                            content={{ button({ ready }) { return ready ? "Upload File" : "Loading..."; } }}
                                        />
                                    )}
                                </div>
                            );
                                ...
                                <tr className={`border-b border-gray-100 ${isLocked ? 'bg-gray-50 text-gray-400' : 'text-gray-900 hover:bg-gray-50 transition-colors'}`}>
                                    ...
                                    <td className="p-4 font-semibold text-gray-800">
                                        {step.type === "PROFILE_CREATION" 
                                            ...
                                            : STEP_LABELS[step.type as ProcedureType]
                                        }
                                    </td>
                                    <td className="p-4">
                                        <span className={`px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full shadow-sm ${
                                            displayStatus === 'Completed' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' :
                                            displayStatus === 'In Progress' ? 'bg-blue-50 text-blue-600 border border-blue-100' :
                                        ...
                                        <td colSpan={5} className="p-6">
                                            <div className="bg-white rounded-[32px] border border-blue-100 shadow-xl p-8 space-y-6">
                                                <div className="flex items-center justify-between">
                                                    <div>
                                                        <h4 className="font-black text-gray-900 uppercase tracking-tighter text-xl">Required Documents Roadmap</h4>
                                                        <p className="text-xs text-gray-500 font-bold mt-1">Provide all mandatory files to proceed to agent review.</p>
                                                    </div>
                                                    <button 
                                                ...
                                                <table className="w-full text-left text-sm mt-4">
                                                    <thead>
                                                        <tr className="border-b border-gray-100 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                                                            <th className="py-4 px-2">Document Category</th>
                                                        ...
                                                        {step4Docs.map(docReq => {
                                                            ...
                                                            return (
                                                                <tr key={docReq.name} className="border-b border-gray-50 group transition-colors hover:bg-gray-50/50">
                                                                    <td className="py-4 px-2 font-black text-gray-700 uppercase text-[10px] tracking-tight">{docReq.name}</td>
                                                                    <td className="py-4 px-2">
                                                                        {uploadedDoc ? (
                                                                            <span className="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full font-black text-[9px] uppercase tracking-widest border border-emerald-100">Uploaded</span>
                                                                        ) : (
                                                                            <span className="bg-red-50 text-red-500 px-3 py-1 rounded-full font-black text-[9px] uppercase tracking-widest border border-red-100">Required</span>
                                                                        )}
                                                                    </td>
                                                                    <td className="py-4 px-2">
                                                                        <div className="flex items-center gap-4">
                                                                            <div className="w-28">
                                                                                {uploadingDocType === docReq.type ? (
                                                                                    <span className="text-blue-500 font-bold text-[10px] animate-pulse">Processing...</span>
                                                                                ) : (
                                                                                    <UploadButton
                                                                                        endpoint="documentUploader"
                                                                                        ...
                                                                                        content={{ button({ ready }) { return ready ? (uploadedDoc ? "Replace" : "Upload") : "..."; } }}
                                                                                    />
                                                                                )}
                                                                            </div>
                                                                            {uploadedDoc && (
                                        ...
                                        <td colSpan={5} className="p-6">
                                            <div className="bg-white rounded-[32px] border border-blue-100 shadow-xl p-10 text-center space-y-6">
                                                ...
                                                <div className="space-y-2">
                                                    <h4 className="font-black text-gray-900 uppercase tracking-tighter text-2xl">Administrative Management</h4>
                                                    <p className="text-sm text-gray-500 font-bold max-w-sm mx-auto leading-relaxed">
                                                        This step (Express Entry / Arrima / Campus France) is handled strictly by the administrative agent. You will be notified automatically via this dashboard once your profile has been successfully created.
                                                    </p>
                                                </div>
                                                <div className="pt-4">
                                                    <span className="bg-blue-50 text-[#1E3A8A] px-6 py-2 rounded-full font-black text-[10px] uppercase tracking-widest border border-blue-100 shadow-sm italic">Status: Monitoring in Progress</span>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                                            ...
                                            <div className="bg-white rounded-[32px] border border-blue-100 shadow-xl p-10 space-y-8">
                                                <div className="flex flex-col md:flex-row items-center justify-between gap-6 border-b border-gray-50 pb-8">
                                                    <div className="text-center md:text-left">
                                                        <h4 className="font-black text-gray-900 uppercase tracking-tighter text-2xl">Diploma Equivalence Support</h4>
                                                        <p className="text-sm text-gray-500 font-bold mt-1">Managed milestone. You may provide supporting documents if requested by your agent.</p>
                                                    </div>
                                                    <button 
                                                ...
                                                <div className="bg-gray-50/50 p-8 rounded-[32px] border border-gray-100 space-y-6">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className="text-[10px] font-black text-[#1E3A8A] uppercase tracking-widest">Optional Submission Portal</span>
                                                    </div>
                                                        ...
                                                        {step.Document?.some((d: any) => d.name === "Equivalence_Proof") ? (
                                                            <div className="flex-1 w-full p-6 bg-white border border-emerald-100 rounded-2xl flex items-center justify-between gap-4 shadow-lg shadow-emerald-50/50">
                                                                <div className="flex items-center gap-4">
                                                                    ...
                                                                    <span className="text-xs font-black text-emerald-700 uppercase tracking-widest">Supporting Proof Submitted</span>
                                                                </div>
                                                                {(() => {
                                                            ...
                                                            <div className="flex-1 w-full">
                                                                <UploadButton
                                                                    ...
                                                                    content={{ button({ ready }) { return ready ? "Upload Supporting Proof" : "Preparing Portal..."; } }}
                                                ...
                                                <div className="bg-blue-50/50 p-6 rounded-2xl border border-blue-100 flex items-start gap-4">
                                                    ...
                                                    <p className="text-[10px] text-[#1E3A8A] font-bold leading-relaxed italic">
                                                        Note: This milestone represents an external administrative process. Final validation and status progression are handled strictly by the administrative oversight team.
                                                    </p>
                                                </div>
                                            </div>
                                        </td>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in fade-in zoom-in duration-300">
                    <div className="bg-white rounded-[40px] p-12 max-w-md w-full shadow-2xl border border-gray-100 space-y-8 text-center ring-1 ring-black/5">
                        ...
                        <div className="space-y-4">
                            <h3 className="text-3xl font-black text-gray-900 tracking-tighter uppercase leading-none">System Notification</h3>
                    ...
                    <div className="bg-white rounded-[40px] p-12 max-w-lg w-full shadow-2xl border border-gray-100 space-y-8 ring-1 ring-black/5">
                        <div className="flex items-center gap-6 border-b-2 border-gray-50 pb-8">
                            ...
                            <div className="space-y-1">
                                <h3 className="text-3xl font-black text-gray-900 tracking-tighter uppercase leading-none">Agent Support</h3>
                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Connect with your case officer</p>
                            </div>
                        </div>
                        
                        <div className="space-y-4">
                            <label className="text-xs font-black text-gray-400 uppercase tracking-[0.2em] ml-2">Assistance Details</label>
                            <textarea 
                        ...
                        <div className="flex gap-4">
                            ...
                            >
                                {isSubmittingAssistance ? "Transmitting..." : "Send Request"}
                            </button>
                        </div>
                    </div>
                </div>
